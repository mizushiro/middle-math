!(function (t, e) {
	...
	class h {
		...
		static GetCordsFromTwoPoint(t, e) {
			...
			for (let a = 1; a <= n; a += 1) h.push({ x: o(t.x, e.x, i, n, a), y: o(t.y, e.y, s, n, a) });
			return h;
		}
	}
	class o extends e {
	...
	class g {
		...
		flipNext(t) {
			this.flip({ x: this.render.getRect().left + 2 * this.render.getRect().pageWidth, y: "top" === t ? 1 : this.render.getRect().height - 2 });
		}
		flipPrev(t) {
	...
	class l {
		...
		calculateBoundsRect() {
			...
			return (
				...
				(this.boundsRect = { left: a, top: s - o / 2, width: 2 * h, height: o, pageWidth: h }),
				t
			);
		}
	...
	class w extends l {
		...
		drawInnerShadow() {
			...
			const g = `\n            display: block;\n            z-index: ${(this.getSettings().startZIndex + 10).toString(10)};\n            width: ${e}px;\n            height: ${2 * t.height
				...
				}) 35%,\n                rgba(0, 0, 0, 0) 100%);\n            transform-origin: ${i}px 100px;\n            transform: translate3d(${n.x - i}px, ${n.y - 100
				}px, 0) rotate(${o}rad);\n            clip-path: ${r};\n            -webkit-clip-path: ${r};\n        `;
		...
		drawOuterShadow() {
			...
			const r = `\n            display: block;\n            z-index: ${(this.getSettings().startZIndex + 10).toString(10)};\n            width: ${this.shadow.width}px;\n            height: ${2 * t.height
				}px;\n            background: linear-gradient(${n}, rgba(0, 0, 0, ${this.shadow.opacity}), rgba(0, 0, 0, 0));\n            transform-origin: ${s}px 100px;\n            transform: translate3d(${e.x - s}px, ${e.y - 100
				}px, 0) rotate(${i}rad);\n            clip-path: ${a};\n            -webkit-clip-path: ${a};\n        `;
